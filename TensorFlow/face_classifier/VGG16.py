...
class net_setup(object):
    ...
    def dense_variable(cls, size, node_name):
        return tf.Variable(
            tf.zeros([size], dtype=tf.float32),
...
class VGG16(net_setup):
    ...
    def dense_node(self, in_node, dense_size, name):
        dense_weight = self.dense_variable(size=dense_size, node_name=name)
    ...
    def architecture(self, data_features):
        in_node = self.input_node(data_features)
        ...
        max_pool3 = self.max_pool(conv3_2, "max_pool3")
        ...
        conv4_2 = self.convolutional_node(conv4_2, self.kernel_1x1, self.depth4, "conv4_3")
        max_pool4 = self.max_pool(conv4_2, "max_pool4")
        ...
        conv5_2 = self.convolutional_node(conv2_2, self.kernel_1x1, self.depth5, "conv5_3")
        max_pool5 = self.max_pool(conv5_2, "max_pool5")
