...
class VGG16(net_setup):
    ...
    def architecture4(self, in_node, keep_prob_node):
        '''
...
model.add(Activation('softmax'))
        ...
        conv1 = self.convolutional_node(in_node, self.kernel_3x3, 32, "conv1")
        print("CONV1: ", conv1)
        conv1= tf.nn.relu(conv1)
        
        flatten_node = flatten(conv1)

        dense1 = self.dense_node(flatten_node, 128, name="dense1")
        dense1 = tf.nn.relu(dense1)

        dense2 = self.dense_node(dense1, 2, name="dense2")
        dense2 = tf.nn.relu(dense2)

        self.__logits_node = dense2
    ...
    def __convert_to_one_hot_encoded_labels(self, labels):
        ...
        one_hot_node = tf.one_hot(labels_placeholder, depth=np.max(labels), dtype=tf.float32)
    ...
    def run_architecture(self):
        ...
        train_labels_node = self.input_node(self.__labels_shape)
        print("self.__feature_shape: {0} | self.__labels_shape: {1}".format(self.__feature_shape, self.__labels_shape))
        print("train_feature_node: {0} | train_labels_node: {1}".format(train_feature_node, train_labels_node))

        # Build VGG16 architecture
        self.architecture4(train_feature_node, self.keep_prob_node)
