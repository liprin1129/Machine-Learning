...
void testShow(int count, torch::Tensor const &imgTensor, torch::Tensor const &labelTensor) {
    ...
    //std::sprintf(outputString, "./checkpoints/Images/output-%03d.jpg", count);
...
{    
    ...
    // Optimizer
    torch::optim::Adam adamOptimizer(fln->parameters(), torch::optim::AdamOptions(5e-5).beta1(0.5));
    ...
    auto cds = CustomDataset(
        ...
        //"/DATASETs/Face/Landmarks/Pytorch-Tutorial-Landmarks-Dataset/faces/",
        false)
        .map(torch::data::transforms::Lambda<torch::data::Example<>>(dataWrangling::Resize(resizeFactor))) // 500
        .map(torch::data::transforms::Lambda<torch::data::Example<>>(dataWrangling::RandomColour(wranglingProb, contrastFactor, brightnessFactor))) // 0.7, 3.0, 50.0
        .map(torch::data::transforms::Lambda<torch::data::Example<>>(dataWrangling::RandomClop(wranglingProb, moveFactor))) // 0.7, 100.0
        .map(torch::data::transforms::Lambda<torch::data::Example<>>(dataWrangling::MiniMaxNormalize()))
        .map(torch::data::transforms::Stack<>());
