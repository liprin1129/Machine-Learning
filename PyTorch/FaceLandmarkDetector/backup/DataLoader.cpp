...
cv::Mat DataLoader::resizeCVMat(cv::Mat &cvImg, int newSize, float scaleFactor) {
    cv::Mat resizedImage;
    
    //std::cout << scaleFactor << " | New Size: " << cv::Size2d((int)(cvImg.cols*scaleFactor), (int)(cvImg.rows*scaleFactor)) << std::endl;
    
    if (newSize > 0)
        cv::resize(cvImg, resizedImage, cv::Size2d(newSize, newSize), 0, 0, cv::INTER_LINEAR);
    else
...
std::tuple<cv::Mat, std::list<float>> DataLoader::loadOneTraninImageAndLabel(std::tuple<std::string, std::string> filePath, bool norm) {
    ...
    if (ptsFile.is_open()) {
        ...
        */
        ...
        image = resizeCVMat(image, 128, scaleFactor); // Resize input image w.r.t. scaleFactor ratio
