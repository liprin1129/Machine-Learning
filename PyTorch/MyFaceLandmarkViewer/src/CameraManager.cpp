...
void CameraManager::displayFrames() {
    ...
    while(true) {
        ...
        {   
            ...
            auto [leftCVMatWithLandmarks, rightCVMatWithLandmarks] = tiPtr->inferStereo(fln, _cvLeftMat, _cvRightMat, torch::Device(torch::kCUDA));
...
void CameraManager::CameraManagerHasLoaded() {
    ...
    tiPtr = std::make_shared<TrainerInferrer>();

    fln = FaceLandmarkNet(3, false);

    torch::load(fln, "/DEVs/Machine-Learning/PyTorch/FaceLandmarkViewer/build/checkpoints/Trained-models/backup-models/model-2000.pt");
