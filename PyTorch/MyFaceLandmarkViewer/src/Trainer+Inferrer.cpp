...
(
    FaceLandmarkNet fln, 
    const cv::Mat &leftCVImg, const cv::Mat &rightCVImg, 
...
) 
{
    ...
    if (leftCVImg.rows > 0 && leftCVImg.cols > 0 && rightCVImg.rows > 0 && rightCVImg.cols > 0)
    {
        ...
        auto leftImageCVMat = leftCVImg.clone(); //dataWrangling::Utilities::TensorToCVMatConverter(leftImageTensor, 500);
        auto rightImageCVMat = rightCVImg.clone(); //dataWrangling::Utilities::TensorToCVMatConverter(rightImageTensor, 500);
        ...
        for (int i=0; i<leftLabels.size(); ++i){
            ...
            if (leftY < leftImageCVMat.rows and leftX < leftImageCVMat.cols) {
                cv::circle(leftImageCVMat, cv::Point2d(cv::Size((int)leftX, (int)leftY)), 3, cv::Scalar( 54, 54, 251 ), cv::FILLED, cv::LINE_4);
            }

            if (rightY < rightImageCVMat.rows and rightX < rightImageCVMat.cols) {
                cv::circle(rightImageCVMat, cv::Point2d(cv::Size((int)rightX, (int)rightY)), 3, cv::Scalar( 54, 54, 251 ), cv::FILLED, cv::LINE_4);
            }
        ...
        return std::make_tuple(leftImageCVMat.clone(), rightImageCVMat.clone());
    }
}