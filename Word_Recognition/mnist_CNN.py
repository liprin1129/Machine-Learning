...
in_true = tf.placeholder(tf.float32, shape=img_shape)
out_true = tf.placeholder(tf.int32, shape=(None))
...
def cnn_dense_model(features):
    ...
    dropout1 = tf.nn.dropout(dense1, drop_prop)
...
logits = cnn_dense_model(in_true)
loss = tf.reduce_mean(
    tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits=logits, labels=out_true))

prediction = tf.nn.softmax(logits, name='softmax_tensor')

def cnn_dense_train(loss):
